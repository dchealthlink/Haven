...
//$ch = curl_init('https://www.dcmic.org/web_serv_resp_obj.php');
$ch = curl_init('http://localhost/web_serv_ridp_obj.php');
...
foreach($deresult as $prop => $value) {
        if ( is_array($value)) {
                ...
                foreach($value as $prop1 => $value1) {
                        if ( is_array($value1)) {
                        echo "<br><br>Level1:".$prop1." ==> 1";
                                foreach($value1 as $prop2 => $value2) {
                                        if (is_array($value2) ) {
                                        echo "<br><br>Level2: ".$prop2." ==> 2";
                                                foreach($value2 as $prop3 => $value3) {
                                                        if (is_array($value3) ) {
                                                        echo "<br><br>Level3: ".$prop3." ==> 3";
...
//						$exresult = execSql($db, $l3asql, $debug) ;
                                                ...
                                                echo '<br>Level2a: '.$prop2.' : '.$value2.'5<br>';
...
//                                echo '<tr><td>Level1: </td><td>'.$prop1.' : </td><td>'.$value1.'</td></tr>';
				switch ($prop1) {
					...
					break;
					default:
                                	echo '<tr><td>'.$prop1.' : </td><td>'.$value1.'6</td></tr><br>';
