...
if ($NEWSEARCH) {
	header("Location: curam_menu.php?calling=".$calling);
}
...
if ($ctable_field) {
	$search_where = " ctable_field = '".$ctable_field."' ";
} else {
	if (!$search_where) {
 		$search_where = "1 = 1 "; 
	}
...
$sql = "SELECT * FROM curam_table_field WHERE ".$search_where;

$sql.=" ORDER BY 1 limit 500";
// echo $sql;

$result = execSql($db, $sql, $debug);

$numrows = pg_numrows($result);
echo ("<tr><td>Field in Table Information</td></tr>");
...
if ($result) {
	...
	for ($i=0;$i<$numrows;$i++) {
			...
			$ftype = pg_field_type($result,$j);
				...
				if ($j == 0) {
					if ($numrows > 1) {
						echo "<td><a href=search_curam_result.php?calling=".$calling."&curam_cd=".$val.">".$val."</a></td>"; 
					} else {
						echo "<td>".$val."</td>";  
					}
					$temp_vehicle_tag_no = $val;
				} else {
					if ($ftype == 'numeric') {
...
if ($numrows>=1) {
	...
	$ctable_field = $rowarr[1];

	$sql="SELECT * from curam_table_fk WHERE ctable_field = '".$ctable_field."'  ";

	$result = execSql($db, $sql, $debug);

	echo ("<tr><td>&nbsp;</td></tr>");
	echo ("<tr><td>Foreign Key Information</td></tr>");
	...
	if ($result) {
		...
		for ($i=0;$i<$numrows;$i++) {
				...
				$ftype = pg_field_type($result,$j);
                		$val = $rowarr[$j];
				if ($j == 0) {
                        		echo "<td><a href=search_curam_result.php?curam_cd=".$val.">".$val."</a></td>";
				} else {
					$ftype = pg_field_type($result,$j);
                			$val = $rowarr[$j];
                			if ($val == "" or $val == "  ") {
...
/* === open_log ==== */
	...
	$sql="SELECT xref_table_name, xref_field, ctable_name, ctable_field  FROM curam_table_fk  WHERE xref_field = '".$ctable_field."' ";
