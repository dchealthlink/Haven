...
//                $innersql = "SELECT pay_day, hours_reg, '100', hours_ot, ot_pay_type_cd, hours_other, other_pay_type_cd FROM employee_pay_log WHERE employee_id = '".$list_employee_id."' ORDER BY employee_id, pay_day ";
                $innersql = "SELECT pay_day, hours_reg, '100', hours_ot, ot_pay_type_cd, hours_other, other_pay_type_cd, meals_count FROM employee_pay_log WHERE employee_id = '".$list_employee_id."' AND pay_period_num = '".$pay_period_num."' ORDER BY employee_id, pay_day";
                $innersql = "SELECT department_id, pay_day, coalesce(coalesce(ot_pay_type_cd, other_pay_type_cd),'100') as pay_type, sum(hours_reg), sum(meals_count),sum(hours_ot),  sum(hours_other) FROM employee_pay_log WHERE  employee_id = '".$list_employee_id."' AND pay_period_num = ".$pay_period_num." GROUP BY department_id, pay_day, 4 ORDER BY 1,2,3";
                ...
                for ($inrows = 0; $inrows < $innerrows; $inrows++) {
                        ...
                        list ($list_pay_day, $list_hours_reg, $list_pay_type_cd, $list_hours_ot, $list_ot_pay_type_cd, $list_hours_other, $list_other_pay_type_cd, $meals_count) = pg_fetch_row($innerresult,$inrows);
/*
                        if ($list_hours_reg) {
	                        ...
	                        $checkval = ($list_pay_day - $period_beginning) + 5;
...
$rowtot = 0;
		...
		/*	
                        	...
                        	$checkval = ($det_pay_day - $period_beginning) + 5;
...
$pdf->AddPage();
    ...
    //Logo
    $pdf->Image('images/wm_building.jpg',10,8,23);
    //Arial bold 15
...
// $data[]=explode(';',$dummy);
                 ...
                 $innersql = "SELECT pay_day, hours_reg, '100', meals_count FROM employee_pay_log WHERE employee_id = '".$list_employee_id."' AND pay_period_num = '".$pay_period_num."' ORDER BY employee_id, pay_day";
                 $innersql = "SELECT department_id, pay_day, '100', sum(hours_reg), sum( meals_count) FROM employee_pay_log WHERE employee_id = '".$list_employee_id."' AND pay_period_num = '".$pay_period_num."' group by 1,2,3 ORDER BY 1,2,3";
...
//			$ctr_set = $ctr;
                        ...
                        list ($list_department_id, $list_pay_day, $list_pay_type_cd, $list_hours_reg,  $list_meals_count) = pg_fetch_row($innerresult,$inrows);
				...
				*/
                                $data_temp[1][0] = $list_department_id;
                                $data_temp[1][1] = $list_last_name;
                                $data_temp[1][2] = $list_first_name;
                                $data_temp[1][3] = $list_employee_id;
                                $data_temp[1][4] = $list_pay_type_cd;
				if ($list_pay_day < $period_beginning) {
                                        $lastdayval = date("z",mktime(0, 0, 0, 12, 31, substr($period_beg_date,0,4)));
                                        $checkval = $list_pay_day + 5 + (($lastdayval + 1) - $period_beginning);
				} else {
	                                $checkval = ($list_pay_day - $period_beginning) + 5;
				}
                                $data_temp[1][$checkval] = $list_hours_reg;
                        }
			if ($list_meals_count) {
				/*
                                $data[$ctr_set + 8][0] = $list_department_id;
                                $data[$ctr_set + 8][1] = $list_last_name;
                                $data[$ctr_set + 8][2] = $list_first_name;
                                $data[$ctr_set + 8][3] = $list_employee_id;
                                $data[$ctr_set + 8][4] = 'Meals';
                                $checkval = ($list_pay_day - $period_beginning) + 5;
                                $data[$ctr_set + 8][$checkval] = $list_meals_count;
				*/
                                $data_temp[0][0] = $list_department_id;
                                $data_temp[0][1] = $list_last_name;
                                $data_temp[0][2] = $list_first_name;
                                $data_temp[0][3] = $list_employee_id;
                                $data_temp[0][4] = 'Meals';
				if ($list_pay_day < $period_beginning) {
                                        $checkval = $list_pay_day + 5 + (($lastdayval + 1) - $period_beginning);
				} else {
                                ...
                                $data_temp[0][$checkval] = $list_meals_count;
			}
...
$rowtot = 0;
			...
			*/
				$checker = 1 + $pay_day_count;
                                $data_temp[(1 + $pay_day_count)][0] = $list_department_id;
                                $data_temp[(1 + $pay_day_count)][1] = $list_last_name;
                                $data_temp[(1 + $pay_day_count)][2] = $list_first_name;
                                $data_temp[(1 + $pay_day_count)][3] = $list_employee_id;
                                $data_temp[(1 + $pay_day_count)][4] = $det_pay_type_cd;
				if ($det_pay_day < $period_beginning) {
                                        $checkval = $det_pay_day + 5 + (($lastdayval + 1) - $period_beginning);
				} else {
                                	$checkval = ($det_pay_day - $period_beginning) + 5;
				}
                                $data_temp[(1 + $pay_day_count)][$checkval] = $det_hours;
				$prev_pay_code = $det_pay_type_cd;
...
/* === dump temp matrix == */
                ...
                for ($tmrows = 0; $tmrows < $temp_ctr; $tmrows++) {
			for ($fldnum = 0; $fldnum < 20; $fldnum++) {
				$data[$ctr][$fldnum] = $data_temp[$tmrows][$fldnum];
			}
			$ctr = $ctr + 1;
		}
/* == end dump temp matrix == */
// $ctr = $ctr + 6;
        }
    ...
    foreach($data as $col)
    {
		...
		if ($oldcol != $col[3] AND $oldcol != "" AND $col[1]) {
                $pdf->Cell($w[0],3.5,'','LR');
