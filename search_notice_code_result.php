...
$sql = "SELECT notice_cd, notice_method_cd, notice_subject, notice_text, notice_template FROM notice_code WHERE ".$search_where;
if ($notice_cd) {
	$sql.=" AND notice_cd = '".$notice_cd."' ";
}
$sql.=" ORDER BY notice_cd limit 1000";

$result = execSql($db, $sql, $debug);

        $numrows = pg_numrows($result);
echo ("<tr><td>Notice Code Information</td></tr>");
if ($numrows > 0) {
...
for ($i=0;$i<$numrows;$i++) {
		...
		$ftype = pg_field_type($result,$j);
			...
			if ($j == 0) {
				if ($numrows > 1) {
					echo "<td><a href=search_notice_code_result.php?notice_cd=".$val.">".$val."</a></td>"; 
				} else {
					echo "<td>".$val."</td>"; 
				}
				$temp_notice_cd = $val;
...
$sql="SELECT notification_id, notice_cd, posting_process, post_timestamp, notice_method_cd FROM notification WHERE notice_cd = '".$notice_cd."' AND pickup_timestamp is NULL ORDER BY post_timestamp desc";
$result = execSql($db, $sql, $debug);
echo ("<tr><td>&nbsp;</td></tr>");
echo ("<tr><td>Pending Notification Information</td></tr>");
...
for ($i=0;$i<$numrows;$i++) {
	...
	echo "<td><a href=search_notice_data_result.php?notification_id=".$rowarr[0].">View Data</a></td>"; 
...
echo "<input type=hidden name=notice_cd value=".$notice_cd.">";
