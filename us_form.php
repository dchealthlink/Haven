        ...
        echo "<td> Links:</td><td colspan=7><table border=".$bordervalue.">";
                        $rownum = 0;
                                while ($relationship_row = pg_fetch_array($relationship_all, $rownum)) {
                                        $item_value = $relationship_row["action_value"];
                                        $item_inc = $relationship_row["action_inc"];
                                echo '<tr><td><a onclick="window.open(\'us_action_form.php?usId='.$userstoryId.'&at=link&ac='.$item_inc.'\',\'name\',\'width=800,height=600\')">'.$item_inc.'</a></td>';
				echo '<td colspan=50 ><a onclick="window.open(\''.$item_value.'\',\'name\',\'width=1100,height=800\')">'.$item_value.'</a></td></tr>';
                                        $rownum = $rownum + 1;
                                }
echo "</table>";
echo "</td>";
        ...
        echo "<td> Cross References:</td><td colspan=7><table border=".$bordervalue.">";
                        $rownum = 0;
                                while ($relationship_row = pg_fetch_array($relationship_all, $rownum)) {
                                        ...
                                        $item_value = $relationship_row["action_value"];
                                        $item_inc = $relationship_row["action_inc"];
                                echo '<tr><td><a onclick="window.open(\'us_action_form.php?usId='.$userstoryId.'&at=xref&ac='.$item_inc.'\',\'name\',\'width=1200,height=600\')">'.$item_name.'</a></td><td colspan=50 >'.$item_value.'</td></tr>';
                                        $rownum = $rownum + 1;
                                }
echo "</table>";
echo "</td>";
...
$rownum = $rownum + 1;
		...
		echo '<tr><td>Actions:</td><td colspan=3 id="actionTypeValueHeader" style="display:none">Value</td>';
echo '<td id="actionTypeUserStoryHeader" style="display:none">User Story';
echo '<td id="actionTypeXrefHeader" style="display:none">Cross Reference';
echo '<td id="actionTypeLinkHeader" style="display:none" colspan="2">Link';
echo '<td valign="middle" id="actionTypeCommentHeader" style="display:none">Comments';
		echo "<input class=asLabel type=text id=currentrelperson name=currentperson readonly></td></tr>";
  		...
  		echo '<td valign=top><select id="actionType" name="actionType" onchange="showActionType();">';
 		echo '<option value="'.$no_prelationship.'" >'.$no_prelationship.'</option>';
 
  			$relationship_all = pg_exec($db,"SELECT item_cd, item_description from app_lookup where lookup_table = 'user_story_action' and lookup_field = 'action_type' order by sort_order "); 
			$rownum = 0;
				while ($relationship_row = pg_fetch_array($relationship_all, $rownum)) {
  					$item_description = $relationship_row["item_description"];
  					$item_cd = $relationship_row["item_cd"];
		...
		echo '<td id="actionTypeLink" style="display:none" valign="top" colspan="2"><input type="text" name="linkvalue" size="40" maxlength="800">';
		echo "</td>";


		echo '<td id="actionTypeUserStory" style="display:none" colspan="3">';
  		echo '<select name="selecteduserStoryId">';
 		echo '<option value="'.$no_prelationship.'" >'.$no_prelationship.'</option>';
 
	
  			$relationship_all = pg_exec($db,"SELECT userstoryid as ucai, userstoryname||'  (tier:'||us_tier||')' as storytype from  user_story   order by 1 "); 
			$rownum = 0;
				while ($relationship_row = pg_fetch_array($relationship_all, $rownum)) {
  					$item_description = $relationship_row["storytype"];
  		...
  		echo "</select>";
		echo "</td>";
//		echo '<td colspan="3" id="actionTypeComment" style="display:none"><TEXTAREA name="comments" rows="3" cols="60"></textarea></td>';

		echo '<td id="actionTypeXref" style="display:none" colspan="3">';
  		echo '<select name="usecaseId">';
 		echo '<option value="'.$no_prelationship.'" >'.$no_prelationship.'</option>';
 
	
  			$relationship_all = pg_exec($db,"SELECT usecaseid as ucai, usecasename||'  (tier:'||uc_tier||')' as casetype from  use_case WHERE usecaseid not in (select action_value from user_story_action WHERE action_type = 'xref' and userstoryid = '".$userstoryId."')   order by 1 "); 
			$rownum = 0;
				while ($relationship_row = pg_fetch_array($relationship_all, $rownum)) {
  					$item_description = $relationship_row["casetype"];
  					$item_cd = $relationship_row["ucai"];
					$rownum = $rownum + 1;
  					echo "<option value='$item_cd'>$item_description</option>";
  				}
...
<?
 echo "<input class=gray type=Button name=FORMNUM value=\"Upload Docs\" onClick=\"window.open('us_doc_file_upload.php?ucid=".$userstoryId."','newWin','scrollbars=yes,status=no,toolbar=no,directories=no,resizable=no,screenX=100,screenY=500,top=100,left=500,width=750,height=400')\"> ";
?>
