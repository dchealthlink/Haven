...
function submitform(form){
  ...
  location="select_aptc_response.php?search_usr_id="+prim;
  }
...
//-->
  ...
  <blockquote>
   <h1>APTC Response Type by User</h1>
...
//	$search_usr_id = $data_matrix[0][4] ;
	for ($k = 0; $k < $numrecs; $k++) {
		if ($data_matrix[$k][0]) {
			$sql = "INSERT INTO  aptc_response_type_list_user values ('".$data_matrix[$k][3]."','".$data_matrix[$k][1]."','Y' )"; 
...
$t_sql = "SELECT user_id, user_name FROM app_user ORDER BY user_name";
...
/* ============ end new ============ */
$sql = "SELECT  a.response_type, a.display_flag, a.user_id, u.user_name  FROM aptc_response_type_list_user a, app_user u  WHERE a.user_id = u.user_id and a.user_id = '".$search_usr_id."' " ;

$list_label="N";
$orient="horizontal";
$link_reference="delete_aptc_response.php?uid=".(urlencode($search_usr_id))."&rtype=";
...
$sql = "SELECT response_type,  display_flag as default, '".$search_usr_id."' as usr_id FROM aptc_response_type_list WHERE coalesce(display_flag,'N') != 'X' and response_type NOT IN (SELECT response_type FROM aptc_response_type_list_user WHERE user_id = '".$search_usr_id."') ORDER BY 1"; 
