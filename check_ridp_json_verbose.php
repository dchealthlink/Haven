...
// $genapikey = gen_trans_id($db, $debug) ;

echo 'get eid is '.$_GET['eid'].'<br>';
echo 'post eid is '.$_POST['eid'].'<br>';
...
echo '<br>datastring is ==  '.$datastring.'<br>';
echo '<br><br>';
}
print_r($post0);
$newstring = json_encode($post0,true);
...
/*
	echo "prop is ".$prop." and   value is  ".$value."<br>";
                for ($j = 0; $j < sizeof($value); $j++) {
	                foreach($value[$j] as $prop1 => $value1) {
				echo "prop 1 is ".$prop1."  and value1 is ".$value1."<br>";
			}
		}
...
*/
		...
		echo "is an array of type ==: ".$prop."<br>";
			...
			echo "value1 is ".$value1."<br>";
                        if ( is_array($value1)) {
					if ($prop == 'field') {
							$e_row = $e_row+1;
				 			echo "p1: ".$prop1." and the row is ".$e_row."<br>";
					}
				// field name is herefield
...
// echo ("</div></td>");
                 		...
                 		echo '<br>Level1: '.$prop0.' : '.$value0."<br>";
                         }
               }
            } else {
                 // echo '<br> not an array?<br>';
//                 echo '<br>Level0a: '.$prop0.' : '.$value0;
                 echo '<br>Level0: '.$prop.' : '.$value;
		$$prop = $value;
         }
...
<!--   <form name="mainform" method="post" action="call_ws_resp_obj.php"> -->
   <form name="mainform" method="post" action="check_ridp_json_verbose.php">
