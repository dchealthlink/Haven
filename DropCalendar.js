...
var DefaultFormat="MMDDYYYY";// Default Format to display date.
...
function OpenCalfrom3(pCtrl,pCtrla,pCtrlb,pCtrlc,pFormat)
{

var mydate= new Date();

if (pCtrla = 99) {
 pCtrla = mydate.getMonth()+1;
}
if (pCtrlb = 99) {
 pCtrlb = mydate.getDate();
}
if (pCtrlc = 99) {
 pCtrlc = mydate.getFullYear();
}
...
var formdate = new Date(months[eval(pCtrla)]+' '+pCtrlb+', '+pCtrlc);
...
// Cal.Ctrl.value = Cal.FormatDate(PValue);
 tempvalue = Cal.FormatDate(PValue);
 dateval = tempvalue.split("/");
...
if (Cal.Ctrl.id == 'start_year') {
 document.all.start_month.value = dateval[0];
 document.all.start_day.value = dateval[1];
 document.all.start_year.value = dateval[2];
} 
if (Cal.Ctrl.id == 'end_year') {
 document.all.end_month.value = dateval[0];
 document.all.end_day.value = dateval[1];
 document.all.end_year.value = dateval[2];
}
 CloseCal();
}
